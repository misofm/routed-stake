...
- **`routed_stake::register` / `unregister`** — parent-gated; binds/unbinds the position to the pool it earns from. `register` refuses the parent's own `RoyaltyPool<PoolShare, Currency>` (`ESelfRoute`): it is `sweep`'s destination, and a position earning from its own destination could never be swept, unregistered, or unstaked. This is an address check, so the same share type staked under a different parent is fine. Unregister requires rewards drained to zero (a final `sweep`) first.
